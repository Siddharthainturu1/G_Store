...
/*
    5. Get Electronics’s Collection
    URL	/product/kids
    Fields	No-fields
    Method	GET
    Access	PUBLIC
 ...
 */
router.get('/electronics', async (request , response) => {
    // Get Electronics’s Collection	Logic
    try {
        let products = await Product.find({category : "ELECTRONICSS"});
        response.status(200).json(products);
    }
    catch (error) {
        console.error(error);
